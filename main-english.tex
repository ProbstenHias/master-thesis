...
The minimizer plugins are named \emph{scipy-minimizer} and \emph{scipy-minimizer-grad}, while the \gls{of} plugins are named \emph{ridge-loss}, \emph{hinge-loss}, and \emph{neural-network}.
...
The \emph{scipy-minimizer} plugin uses this function to minimize the loss function without the gradient.
...
The \emph{scipy-minimizer-grad} plugin uses the same function to minimize the loss function with the gradient.
...
This plugin provides the option to calculate the gradient of the loss function, which can be used together with the \emph{scipy-minimizer-grad} plugin.
...
The key concept of the first approach is that the plugins are as loosly coupled as possible, meaning that as few data as possible is passed between the plugins and the data isntead data should always be retrieved from the database if possible.
...
Exapmlaray the input schema of the \emph{calc\_loss} endpoint is shown in Listing \ref{lst:calc_loss_schema}.
For the minimer plugin that means that at no point it needs to know the hyperparameters of the \gls{of} or event the data.
That results i a sparce input schema for the \emph{minimization} endpoint, which is shown in Listing \ref{lst:minimization_schema}.
...
The input schema of the \emph{calc\_loss} endpoint is shown in Listing \ref{lst:calc_loss_schema_multistep}, while the input schema of the \emph{minimization} endpoint is shown in Listing \ref{lst:minimization_schema_multistep}.
