...
Beisel et al. \cite{Beisel2023} describe how \glspl{vqa} can be implmeneted in a workflow-based system.
They go into detail of how to the quantum part of the algorithm can be implemented in a workflow-based system and focus less on the interaction between the different components.
In their work they split the traditional computing part of the algorithm into two services, the \textbf{Objective Evaluation Service} and the \textbf{Optimization Service}.
Similar to this thesis the Optimization Service also emply SciPy to perform the optimization.

In a succeeding paper \cite{Beisel2023a} they discribe their architecture in more detail, showing how the different components interact with each other.
They describe how the different compoenents of the system are implemented as microservices that provide a \gls{rest}ful API to interact with.
The \textbf{Quokka Gateway} acts as a central component that unites the different microservices and provides a single \gls{rest}ful interface to interact with the system.



\cite{Thullier2021} \cite{Mayer_2003}
