...
To preserve the continuity of information across different plugin endpoints, the implementation leverages the multi-step plugin concept inherent to \gls{qhana}.
...
Importantly, the endpoints URL incorporates the database task's ID.
...
To achieve this, an adaptation of \gls{qhana}'s \emph{add\_step} function was developed.
...
To manage this standardized data exchange, marshmallow schemas serve as the backbone of the implementation.
...
If a plugin wishes to invoke another plugin's interaction endpoint, it can populate the input schema and subsequently parse the response using the output schema.
...
Another implementation challenge revolves around ensuring that plugins can easily discover the endpoints of other plugins, but first a custom marshmallow field for data is introduced.
...
It aids not only in data transfer between endpoints but also ensures efficient data storage in the database.
...
Each list entry is a dictionary consisting of a \textbf{type} and an \textbf{href} field.
...
This dynamic nature is captured in the metadata by making the \textbf{href} field a format string.
...
The invoking plugin is tasked with replacing the placeholder with the actual database ID when known.
...
To illustrate the capabilities of the plugin-based optimization framework, the development encompassed two distinct minimizer plugins and three \gls{of} plugins.
...
The \emph{scipy-minimizer} plugin leverages this method to minimize the loss function, without the need for a gradient.
...
Diverging from these, the \emph{neural-network} plugin crafts a neural network featuring a single hidden layer.
...
The developmental journey resutlet in two distinct methodologies for constructing a plugin-based optimization framework.
This section delineates the intrinsic differences that characterize each strategy.

\subsection{Decoupled Plugin Approach}
\label{sec:firstApproach}

The foundational ethos of the first approach is promoting maximal decoupling between plugins which particularly applies to the \gls{of} plugin.
This entails minimizing data transfer between plugins and emphasizing data retrieval directly from the database whenever feasible.

Specifically, the \gls{of} plugin archives its hyperparameters in the database.
When the \emph{pass\_data} endpoint is invoked, both input and target data are storage in the database.
Thus, for the \emph{pass\_data} endpoint, which determines the number of input weights, only input and target data are directly conveyed, while hyperparameters are fetched from the database.
The loss caluclation endpoints \emph{calc\_loss}, \emph{calc\_grad}, and \emph{calc\_loss\_and\_grad} solely need the weights as input, leveraging already stored hyperparameters and data.
An exemplary schema for the \emph{calc\_loss} endpoint is illustrated in Listing \ref{lst:calc_loss_schema}.

Consequently, for the minimizer plugin, there's no occasion where knowledge of the \gls{of}'s hyperparameters or its data is essential, leading to a lean input schema for the \emph{minimization} endpoint, as depicted in Listing \ref{lst:minimization_schema}.
However, it mandates that the task's database ID be communicated through the microfrontend callback process to the coordinator plugin, which is essential for managing interaction endpoint URLs, as discussed in \ref{sec:metadataEndpoint}.

\subsection{Integrated Plugin Approach}
\label{sec:secondApproach}

The second strategy embodies a tighter integration.
In this approach, the \gls{of} plugin is not implemented as a multi-step plugin.
This implies that no shared context exists among the plugin's endpoints, necessitating the conveyance of all required data during each endpoint invocation.

In practice, this requires the microfrontend callback process to relay the hyperparameters to the coordinator plugin.
The coordinator then transmits both the hyperparameters and data to the \gls{of} plugin's \emph{pass\_data} endpoint.
In this setup, the minimizer plugin also needs know of the hyperparameters and data since it must relay this to the \gls{of} plugin's loss caluclation endpoint.
...
To uphold a degree of generalization across processes, irrespective of the \gls{of}'s internal mechanics, hyperparameters are channeled as a dictionary.
This ensures that neither the coordinator nor the minimizer plugin requires in-depth knowledge of the \gls{of} plugin's specific hyperparameters.
As a trade-off, there's no need to relay a task's database ID to the coordinator.
