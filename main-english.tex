...
One of the standout features of \glspl{ramp} is their ability to seamlessly integrate \gls{ui} components.
Unlike traditional plugins that might require significant adjustments to fit into an application's existing \gls{ui}, \glspl{ramp} come equipped with microfrontends.
...
In essence, \glspl{ramp} represent a paradigm shift in how plugins are perceived and integrated within applications.
...
This section outlines the method developed in this work for creating modular \glspl{vqa} consisting of multiple plugins.
...
\begin{itemize}
  \item Which plugin is tasked with calculating the loss function and which one with the minimization process?
  ...
  \item And lastly, Which one oversees and coordinates the entire process?
\end{itemize}
...
The notion of interface contracts in microservice architectures is not new and is inspired by the OpenAPI specification .
