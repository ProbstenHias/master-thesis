...
In the context of VQAs, this thesis aims to extend the capabilities of QHana by developing a novel approach where plugins can interact with each other.
By leveraging the principles of quantum computing and optimization, VQAs can be implemented as interchangeable plugins within the QHana platform.
This means that different components of a VQA, such as the minimizer and the objective functions, can be developed as separate plugins.
...
\begin{itemize}
  ...
  \item \textit{plugins/optimization/objective\_functions} – Where each objective function plugin occupies its respective subfolder.
