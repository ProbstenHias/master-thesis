...
Interactivity in \gls{qhana} encompasses various facets:
...
Drawing inspiration from existing systems, the interaction between the minimization and objective functions in implementations like Scipy's \emph{optimizer.minimize} \cite{Virtanen2020} function serves as a precedent.
...
The architectural design process is a critical phase that dictates how the components of the solution will function and interaract with each other.
The methodology undertaken for the architectural design of the service-based optimization framework consisted of a series of well-defined steps, ensuring clarity, modularity, and extensibility.

\subsection{Decomposition into Plugins}
The first step involved decomposing the optimization process into distinct plugins.
This was achieved by reviewing relevant literature to understand common practices and methodologies in optimization \cite{Virtanen2020, Nocedal2006, ShalevShwartz2014, Weinan2017}.
The main challenge was to find a good decomposition that on the one hand all functionalities that should later be interchangeable should be encompassed in an extra plugin,
but on the other hand too many plugins would create an overhead that would lead to inefficiency because of commutation overhead.
...
Some challenges in this phase included the following:
\begin{itemize}
  \item Which plugin should be responsible for the calculation of the loss function?
  \item Which plugin should be responsible for doing the minimization?
  \item Which plugin should ask the user for the \gls{of} hyperparameters?
  \item Which plugin should ask the user for the minimization function hyperparameters?
  \item Which plugin should ask the user which \gls{of} and which minimization algorithm to use?
  \item Which plugin should ask the user for the input data?
  \item Which plugin should ask the user for the target variable?
  \item Which plugin should coordinate the whole process?
\end{itemize}
...
It could also invoke another plugin and then stop execution all together, while providing the invoked plugin with a callback URL that the invoked plugin could call once it completes its tasks.
...
Each diagram encapsulated the primary tasks of a plugin and the interfaces it provides, offering a clear understanding of the system's structure.


\subsection{Sequence Diagrams and Interaction Flow}
...
Throughout the design odyssey, continuous feedback loops were integrated. This involved:
\begin{itemize}
\item Revisiting each stage, evaluating its alignment with overarching goals.
\item Making necessary refinements to ensure the architecture was both robust and flexible.
\end{itemize}

With the architectural design strategy in place, the next step was to define the implementation strategy.
...
The implementation section of this thesis will ultimately present two distinct versions of a service-based optimization framework.
The evaluation will assess both versions to facilitate a comparison between the two approaches.
Additionally, the evaluation will juxtapose these approaches with a non-service-based approach, presented in the form of a Jupyter notebook.
...
Performance is paramount, especially in a service-based architecture.
A direct comparison will be made between the two service-based approaches and the non-service-based approach.
...
This evaluation hinges on quantitative metrics, with results graphically depicted for enhanced clarity. The \emph{time.perf\_count} function from Python is employed to capture measurements, while the Google Chrome developer tools are used to measure network traffic.

\subsection{Interchangeability}
A litmus test for modularity is its potential for component interchangeability without disruptions. Accordingly, every plugin of a particular type is tested against every other plugin of the same type to validate seamless interchangeability.

\subsection{Standardization Adherence}
Standardization, pivotal for ensuring compatibility and uniformity across diverse plugins, is put to the test. An evaluation is undertaken to ascertain whether all implemented plugins conform to the prescribed standards. This guarantees that all optimization plugins can be implemented uniformly, facilitating consistent and compatible integrations in the future.

\subsection{User Experience}
While the primary audience for this thesis is developers for the \gls{qhana} platform, the user experience is not to be undermined. An assessment determines the ease and efficiency with which a developer can introduce a new plugin in the context of service-based optimization in \gls{qhana}. The lines of code required to integrate a new, interchangeable plugin for the optimization process are enumerated and compared across the approaches.
...
A minimal version of the code that was used to generate the test data can be found in the appendix \ref{appendix:generate_data}.
...
By employing such multifaceted and representative datasets, the methodology aims to guarantee a holistic evaluation of the optimization solutions, assessing their performance, scalability, and robustness across diverse conditions.
...
This interface is also in accordance with the \gls{qhana} documentation \cite{Buehler2022}.
\item \textbf{PassDataOF}: This endpoint is used to pass the input data and the target data to the \gls{of} plugin.
...
\begin{itemize}
  ...
  \item \textbf{(Optional) PassDataMinimizer}: Similar to the \texttt{PassDataOF} endpoint, this endpoint is used to pass the input data and the target data to the minimizer plugin.
  As the a minimizer does not necessarily need the data, this endpoint is optional.
  The nescesscity of this endpoint is the difference in two alternative approaches that will be discussed in the following sections.
  For now we just assume that the minimizer does not need the data.
...
The \gls{of} plugin provides interaction endpoints of types \emph{objective-function\_calc}, \emph{objective-function\_gradient}, \emph{of-loss-and-grad}, and \emph{of_pass_data}.
Similarly, the minimizer plugin offers the `minimization-step' type and optionally the `min-pass-data' type.
These interaction endpoints correpsond to the endpoints defined in the previous section.
...
\begin{description}
    ...
    \begin{itemize}
        ...
        \begin{description}
            ...
            \item[Optional:] URL to \emph{CalculateGradient}, URL to \emph{CalculateLossAndGradient}, URL to callback enpoint of invoking plugin
        \end{description}
    ...
    \begin{itemize}
        ...
        \begin{description}
            \item[Required:] x, y
        \end{description}
        \item \textbf{Returns:} OK
    \end{itemize}

    \item[objective-function\_calc]
    \begin{itemize}
        \item \textbf{Arguments:}
        \begin{description}
    ...
    \item[objective-function\_gradient]
    \begin{itemize}
        \item \textbf{Arguments:}
        \begin{description}
            \item[Required:] weights for the loss function
...
With the architecure now fully defined, the final interaction flow can be visualized.
Figure \ref{fig:interaction_flow} shows the interaction flow for the two proposed approaches.


\chapter{Implementation}
\label{chap:implementation}
here we describe how we implemented the architecture

how the uis of the pluings look like mostly how we defined them in the marshmallow schemas
in order to show only of and min in the correct form we have the plugins labels

what final methods the coordiantor plugin has
...
in one the data itself it not passed to the calc_loss enpoint but instead the data is loaded from the db in the calc_loss endpoint
in the other approach the data is passed to the calc_loss endpoint and not loaded from the db
