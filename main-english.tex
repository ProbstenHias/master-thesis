...
\subsection{Objective Function Plugin UI}
For the \gls{of} plugin, the user inputs the hyperparameters needed by the loss function.
As an illustration, let's consider the Ridge loss function.
The hyperparameters is `alpha`.
The corresponding marshmallow schema for the resulting schema is shown in \lstinputlisting[language=Python, caption=Hyperparameter Schema for Ridge Loss Function UI., label=lst:hyperparameter_schema_ridge]{code/hyperparameter_schema_ridge.py}

\subsection{Minimizer Plugin UI}
The minimizer plugin's UI is primarily concerned with the choice hyperparameters for the minimization function.
Here's the schema definition for a minimizer plugin that uses Scipy as the minimization algorithm and requires the hyperparameter \emph{method} to be specified:

\lstinputlisting[language=Python, caption=Hyperparameter Schema for Scipy Minimizer UI., label=lst:hyperparameter_schema_scipy]{code/hyperparameter_schema_scipy.py}

\subsection{Coordinator Plugin UI}
...
The corresponding schema is shown in \lstinputlisting[language=Python, caption=Coordinator Plugin UI Schema., label=lst:coordinator_schema]{code/ui_schema_coordinator.py}.

A noteworthy feature is the utilization of the \emph{PluginUrl} class for the fields related to plugin selection.
...
\section{Implementation of Interaction Endpoints Metadata}
