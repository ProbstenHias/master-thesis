...
\item \textbf{HRef}: This endpoint is used to process the input from the \gls{of} microfrontend and is therefore ususally called the \textbf{processing} endpoint.
...
\begin{itemize}
    ...
    The coordintor plugin provides a callback endpoint to the \gls{of} plugin that is called once the user entered the input and the input was processed.
...
Here, the coordinator plugin retrieves the user-selected \gls{of} and minimztion plugin metadata, including their interaction endpoints.
...
The coordinator then triggers the minimization endpoint, initiating a long-runnging minimization task that continuously calls the \gls{of} calculation endpoint.
...
The resulting code that realized two different approaches to a plugin-based optimization formawork can be found in \gls{qhana}'s GitHub repository \cite{qhanaGithub}.
...
As the \glspl{ui} of the \gls{of} and minimizer plugins are desinged to be invoked by the coordinator plugin, their \emph{UIHref} endpoint accept a \emph{Callback URL} as query parameter.
